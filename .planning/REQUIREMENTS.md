...
- [ ] **LIB-01**: User can save currently playing or browsed track to Liked Songs via context menu action
- [ ] **LIB-02**: User can remove track from Liked Songs via context menu action (Unlike)
- [ ] **LIB-03**: Track context menu shows current liked state — displays "Like" or "Unlike" based on library check
- [ ] **LIB-04**: Liked state check uses `GET /me/library/contains` without adding noticeable delay to menu rendering
- [ ] **LIB-05**: Plugin requests `user-library-modify` and `user-library-read` scopes; upgrade triggers one-time re-auth via cacheSchemaVersion bump
