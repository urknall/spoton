# Requirements Archive: v1.1 Hardening & Reach
...
- [x] **CLEAN-01**: Alle deutschen Kommentare in Perl-Quellcode durch englische ersetzt
- [x] **CLEAN-02**: Alle deutschen Log-Strings (DEBUGLOG, INFOLOG etc.) durch englische ersetzt
- [x] **CLEAN-03**: Verifizierung: `grep -rn` auf deutsche Sonderzeichen in Code-Kommentaren liefert null Treffer (i18n strings.txt ausgenommen)
...
- [x] **META-01**: Songinfo zeigt "(Spotify Browse)" oder "(Spotify Connect)" je nach aktivem Modus
- [x] **META-02**: Songinfo zeigt aktuelles Stream-Format (OGG, FLAC, MP3, PCM)
- [x] **META-03**: Songinfo zeigt Bitrate wenn verfügbar (z.B. "320k, OGG Vorbis (Spotify Connect)")

### Deployment

- [x] **DEPLOY-01**: GitHub Repo stiefenm/spoton ist öffentlich; interne Dateien (.planning/, .claude/, CLAUDE.md) via .gitignore ausgeschlossen
- [x] **DEPLOY-02**: install.xml existiert mit korrekter Version, Creator und Modulpfaden
- [x] **DEPLOY-03**: repo.xml hat echte SHA1 und Download-URL; LMS zeigt SpotOn in der Plugin-Liste wenn die URL als Custom Repo hinterlegt wird
- [x] **DEPLOY-04**: SpotOn ist auf dem Pi (192.168.13.5) installiert und läuft über den LMS Plugin Manager
- [x] **DEPLOY-05**: SpotOn-Monitor ist aktiv auf dem Pi (Cron-Job, tägliche Log-Rotation); alte Spotty-Artefakte aufgeräumt
...
| ARCH-05 | Phase 8 | Deferred (no macOS CI runner) |
| ARCH-06 | Phase 8 | Deferred (no macOS CI runner) |
| ARCH-07 | Phase 8 | Complete |
| ARCH-08 | Phase 8 | Complete |
| ARCH-09 | Phase 8 | Deferred (human verification pending) |
...
| DEPLOY-01 | Phase 9.5 | Complete |
| DEPLOY-02 | Phase 9.5 | Complete |
| DEPLOY-03 | Phase 9.5 | Complete |
| DEPLOY-04 | Phase 9.5 | Complete |
| DEPLOY-05 | Phase 9.5 | Complete |

**Coverage:**
- v1.1 requirements: 27 total
- Complete: 24
- Deferred: 3 (ARCH-05, ARCH-06, ARCH-09)
- Unmapped: 0 ✓
